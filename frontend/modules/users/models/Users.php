...
class Users extends ActiveRecord implements IdentityInterface,UserRbacInterface
{
  ...
  public function getBalance()
  {
    if (!$this->balans) {
      ...
      $bl['withdraw_count'] = $this->withdrawCount(true);
      $this->balans = $bl;
    }
    return $this->balans;
  }
  ...
  /**
   * @param bool $lastDay - за последние сутки
   * @return int|string - количество выводов средств
   */
  public function withdrawCount($lastDay = false)
  {
    $withdrawCount = UsersWithdraw::find()->where(['user_id' => $this->uid]);
    if ($lastDay) {
      $withdrawCount = $withdrawCount
        ->andWhere(['>', 'request_date', date('Y-m-d H:i:s', strtotime('1 days ago'))]);
    }
    return $withdrawCount->count();
  }
}
