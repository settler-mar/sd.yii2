...
', '', $this->description);
  ...
  public static function makeOrUpdate($coupon)
  {
    ...
    //ddd($coupon);
    if ($result['status'] && !empty($coupon['categories']) && $result['new']) {
      if ($coupon['cpa_id'] == 1) {
        CouponsToCategories::deleteAll(['coupon_id' => $dbCoupon->uid]); //чистим старые связки купон-категория
  ...
  public function getActive(){
    return strtotime($this->date_end)<time() ?0:1;
  }
  ...
  {
    $result = isset(Yii::$app->params['coupons_languages_arrays'][Yii::$app->params['lang_code']]) ?
