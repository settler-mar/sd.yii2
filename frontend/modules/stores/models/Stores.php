...
//    }
  ...
  public static function toFilter($params, $cashName = false)
  {
    $cache = \Yii::$app->cache;
    if ($cashName == false) $cashName = time();

    return $cache->getOrSet($cashName, function () use ($params) {
      $data = self::items()
      //(['name', 'uid', 'priority', 'logo', 'route'])
          //->alias('s')
          //->select(['s.name', 's.uid', 's.priority', 's.logo', 's.route'])
          //->where(['s.is_active' => [0, 1]])
          ->asArray();
      if (!empty($params['where'])) {
        $data->andWhere($params['where']);
      }
      if (!empty($params['sort'])) {
        $data->orderBy($params['sort']);
      }
      return $data->all();
    });
  }
