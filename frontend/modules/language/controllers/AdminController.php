    ...
    public function actionIndex()
    {
      ...
      foreach ($file_php_list as $file){
        ...
        foreach ($lg_list as $lg_k => $lg){
          ...
          if(!is_readable($path)){
            $data['lg'][$lg_k][$file]['WARNING'][]='Фаил отутствует';
            continue;
          }
          $transl=require($path);
          ...
          foreach ($base as $k => $item){
            if(!isset($transl[$k])){
              $data['lg'][$lg_k][$file]['WARNING'][]='Параметр <b>'.$k.'</b> отутствует';
              continue;
            }

            if(trim($transl[$k])==$item){
              $data['lg'][$lg_k][$file]['NOTICE'][]='Параметр <b>'.$k.'</b> совпадает с оригинальным языком';
              continue;
            }

            if(strlen(trim($transl[$k]))<2){
              $data['lg'][$lg_k][$file]['ERROR'][]='Параметр <b>'.$k.'</b> не заполнен';
              continue;
            }
          }
        }
