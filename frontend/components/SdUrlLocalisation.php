...
class SdUrlLocalisation implements UrlRuleInterface
{
  ...
  public function parseRequest($manager, $request)
  {
    ...
    $region_code = isset(Yii::$app->params['regions_list'][$region_code]) ? $region_code : 'ru';
    $region = Yii::$app->params['regions_list'][$region_code];

    $location['areas'] = !empty($region['areas']) ? $region['areas'] : [];
    ...
    $prefix = $path[0];
    if (in_array($prefix, [//эти url не нужно редиректить
        'admin',
        'admin-categories',
        'admin-values',
        'admin-stores',
