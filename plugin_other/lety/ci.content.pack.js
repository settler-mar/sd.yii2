...
window.AddonsFramework.prototype.Browser = function() {
  ...
  var b = navigator.userAgent.match(/(opr|yabrowser|mrchrome|chrome(?!.*(?:opr|yabrowser|mrchrome)))\/?\s*(\d+)?/i);
  if (b) {
...
window.AddonsFramework.prototype.Extension = function() {
  ...
  this.fireEvent = function(a, b, c) {
    ...
    if (d[a])
      for (var e = 0; e < d[a].length; e++) d[a][e].call(f, b, c)
  };
  ...
  this.detachEvent = function(a, b) {
    if (d[a])
      for (var c = 0; c < d[a].length; c++) b && b === d[a][c] && (delete d[a][c], d[a][c] = null, d[a].splice(c, 1))
  };
  this.log = function() {
  ...
  this.setItem = function(a, b) {
    if ("string" == typeof a)
      if ("undefined" == typeof b) chrome.storage.local.remove(a);
      else {
        var c = {};
        c[a] = b;
        chrome.storage.local.set(c)
      } else "object" == typeof a && chrome.storage.local.set(a)
  };
  this.getItem = function(a, b) {
    "string" == typeof a || ("array" == typeof a || "object" == typeof a) && 0 < a.length ? chrome.storage.local.get(a, function(c) {
      "string" == typeof a ? b(c ? c[a] : null) : b(c)
    }) : b(null)
  };
  chrome.runtime.onMessage.addListener(function(a, b, c) {
    if ("event" == a.action && (b = a.name, a = {
        data: a.data.data
      },
        d[b]))
      for (var e = 0; e < d[b].length; e++) d[b][e] && d[b][e].call(f, a, c)
  })
};
