...
//        //$siteUrlHref = $siteUrl->getAttribute('href');
  ...
  private function writeStore($merchant, $link)
  {
    ...
    if ($link) {
      ...
      if (!isset($linkArr['scheme'])) $linkArr['scheme'] = "";
      if (!isset($linkArr['host'])) {
        $linkArr['host'] = str_replace($linkArr['scheme'] . '://', '', $link['landURL']);
        $linkArr['host'] = explode('/', $linkArr['host']);
        $linkArr['host'] = trim($linkArr['host'][0], 'wwww.');
      }
